...
let images=localStorage.getItem("imagesAlpha");

if(user){
	let persedUser=JSON.parse(user);
	document.getElementById("pl1").innerHTML=persedUser.name
}
	...
	if (carte1 && carte2) {
		if (tableauCartes[parseInt(carte1.id)] === tableauCartes[parseInt(carte2.id)]) {
			...
			setTimeout(() => {
				...
				carte1.setAttribute("src", "ressources/question.svg");
				carte2.setAttribute("src", "/ressources/question.svg");
...
function retournerCarte(event) {
	...
	if (carte.classList.contains("flip")) {
		...
		carte.setAttribute("src", "ressources/question.svg");
...
function victoire() {
    ...
    if (cartes.length === tableauCartes.length) {
		...
		document.getElementById("sc1").innerHTML=compteurCoups;
		let date=new Date();
		document.getElementById("da1").innerHTML=date.toISOString().split("T")[0];
    }
	compteurCoups = -1;
}
